"""
Report Agent服务
使用LangChain + Zep实现ReACT模式的职业平行宇宙人生报告生成

功能：
1. 根据模拟需求和Zep图谱信息生成平行人生报告
2. 先规划目录结构，然后分段生成
...
logger = get_logger('careerverse.report_agent')
...
class ReportConsoleLogger:
    ...
    def _setup_file_handler(self):
        ...
        loggers_to_attach = [
            'careerverse.report_agent',
            'careerverse.zep_tools',
        ]
...
【广度搜索 - 获取平行人生全貌视图】
这个工具用于获取平行宇宙模拟的完整全貌，特别适合了解平行人生演变过程。它会：
1. 获取所有相关节点和关系
...
【深度采访 - 平行世界人物访谈（双平台）】
调用OASIS模拟环境的采访API，对平行世界中的人物进行真实访谈！
这不是LLM模拟，而是调用真实的采访接口获取平行世界人物的原始回答。
默认在Twitter和Reddit两个平台同时采访，获取更丰富的人生视角。

功能流程：
1. 自动读取人设文件，了解所有平行世界中的人物
2. 智能选择与访谈主题最相关的人物（如同事、导师、家人、竞争对手等）
3. 自动生成访谈问题
4. 调用 /api/simulation/interview/batch 接口在双平台进行真实采访
5. 整合所有访谈结果，提供多视角人生叙事

【使用场景】
- 需要从不同角色视角了解平行人生中的选择和后果（同事怎么看？家人怎么说？导师怎么评价？）
- 需要收集平行世界中多方人物的意见和感受
- 需要获取平行世界人物的真实回答（来自OASIS模拟环境）
- 想让报告更生动，包含"人物访谈实录"

【返回内容】
- 被访谈人物的身份信息
- 各人物在Twitter和Reddit两个平台的访谈回答
- 关键引言（可直接引用）
- 访谈摘要和多角度人生对比
...
PLAN_SYSTEM_PROMPT = """\
你是一个「平行人生报告」的叙事者，拥有对平行宇宙的「人生观察者视角」——你可以洞察平行世界中每一位人物的行为、选择和命运。

【核心理念】
我们构建了一个平行宇宙，并向其中注入了特定的「模拟需求」作为命运分叉点。平行宇宙的演化结果，就是另一个可能的人生故事。你正在观察的不是"实验数据"，而是"另一个可能的人生"。

【你的任务】
撰写一份「平行人生报告」，回答：
1. 在这个平行宇宙中，用户做出了怎样不同的人生选择？
2. 平行世界中的人物是如何与用户互动、做出反应的？
3. 这个平行人生揭示了哪些与现实人生的关键差异和人生转折点？

【报告定位】
- ✅ 这是一份平行宇宙人生叙事报告，讲述"如果当初选择了另一条路，人生会怎样"
- ✅ 聚焦于平行人生走向、平行世界人物反应、人生转折点、与现实的关键差异
- ✅ 平行世界中的人物的言行就是对另一种人生的真实展现
- ❌ 不是对现实世界现状的分析
- ❌ 不是泛泛而谈的职业规划综述
...
【平行人生场景设定】
我们向平行宇宙注入的命运分叉点（模拟需求）：{simulation_requirement}

【平行宇宙规模】
- 平行世界中的实体数量: {total_nodes}
- 人物间产生的关系数量: {total_edges}
- 人物类型分布: {entity_types}
- 活跃人物数量: {total_entities}

【平行宇宙中的部分人生事实样本】
{related_facts_json}

请以「人生观察者视角」审视这个平行人生：
1. 在这个命运分叉点下，平行人生呈现出了怎样的状态？
2. 平行世界中的人物是如何互动和做出选择的？
3. 这个平行人生揭示了哪些与现实的关键差异和人生转折点？

根据平行人生结果，设计最合适的报告章节结构。

【再次提醒】报告章节数量：最少2个，最多5个，内容要精炼聚焦于核心人生叙事发现。"""

# ── 章节生成 prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
你是一个「平行人生报告」的叙事者，正在撰写报告的一个章节。

报告标题: {report_title}
报告摘要: {report_summary}
平行人生场景（模拟需求）: {simulation_requirement}
...
这是另一个可能的人生。我们向平行宇宙注入了特定的命运分叉点（模拟需求），
平行世界中人物的选择和互动，展现了另一条人生道路的可能性。

你的任务是：
- 揭示在这个命运分叉点下，平行人生发生了什么
- 叙述平行世界中的人物如何做出选择、如何互动
- 发现与现实人生的关键差异、人生转折点和深意

❌ 不要写成对现实世界现状的分析
✅ 要聚焦于"如果当初选了另一条路会怎样"——平行宇宙的结果就是另一种人生可能
...
1. 【必须调用工具观察平行世界】
   - 你正在以「人生观察者视角」观察另一个可能的人生
   - 所有内容必须来自平行世界中发生的事件和人物言行
   - 禁止使用你自己的知识来编写报告内容
   - 每个章节至少调用3次工具（最多5次）来观察平行宇宙，它代表了另一种人生可能

2. 【必须引用平行世界人物的原始言行】
   - 平行世界中人物的发言和选择是另一种人生可能的真实展现
   - 在报告中使用引用格式展示这些人生片段，例如：
     > "在平行世界中，某个人物表示：原文内容..."
   - 这些引用是平行人生叙事的核心证据
...
4. 【忠实呈现平行人生结果】
   - 报告内容必须反映平行世界中的真实模拟结果
   - 不要添加平行世界中不存在的信息
   - 如果某方面信息不足，如实说明

5. 【对比现实人生与平行人生】
   - 在叙事中适当对比用户的现实人生轨迹与平行宇宙中的发展
   - 突出关键分叉点和选择差异
   - 帮助读者理解"如果当初做了不同选择"的真正含义
...
【正确示例】
```
在这个平行宇宙中，用户选择了一条截然不同的职业道路。通过深入观察平行世界，我们发现...

**初入新行业**

在这个平行世界里，用户从互联网行业转向了教育领域：

> "平行世界中的前同事回忆道：当时所有人都很惊讶..."

**人生的关键转折**

平行宇宙中的职业发展呈现出完全不同的轨迹：
...
- insight_forge: 深度洞察分析，自动分解问题并多维度检索平行世界事实和关系
- panorama_search: 广角全景搜索，了解平行人生全貌、时间线和人生演变过程
- quick_search: 快速验证某个具体人生细节
- interview_agents: 访谈平行世界中的人物，获取不同角色的第一人称视角和真实感受
...
输出你的思考，然后用以下格式调用一个工具：
 ensured{{"name": "工具名称", "parameters": {{"参数名": "参数值"}}}} ensured
...
1. 内容必须基于工具检索到的平行世界数据
2. 大量引用平行世界人物的原文来展现另一种人生
...
4. 【引用格式规范 - 必须单独成段】
   ...
   ✅ 正确格式：
   ```
   在平行世界中，用户选择了一份完全不同的职业。

   > "平行世界中的导师这样评价：这是我见过的最勇敢的职业转型。"

   这一评价反映了平行世界中人们对此选择的普遍看法。
   ```

   ❌ 错误格式：
   ```
   在平行世界中，用户选择了一份完全不同的职业。> "平行世界中的导师这样评价..." 这一评价反映了...
   ```
5. 保持与其他章节的逻辑连贯性
...
请开始：
1. 首先思考（Thought）这个章节需要什么平行世界信息
2. 然后调用工具（Action）获取平行人生数据
...
class ReportAgent:
    """
    Report Agent - 平行人生报告生成Agent

    采用ReACT（Reasoning + Acting）模式：
    1. 规划阶段：分析模拟需求，规划平行人生报告目录结构
    ...
    def __init__(
        ...
        Args:
            graph_id: 图谱ID
            simulation_id: 模拟ID
            simulation_requirement: 平行人生场景描述（模拟需求）
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "interview_agents": {
                ...
                "parameters": {
                    "interview_topic": "访谈主题或需求描述（如：'了解平行世界中同事对用户转行的看法'）",
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # 深度访谈 - 调用真实的OASIS采访API获取平行世界人物的回答（双平台）
    ...
    def plan_outline(
        ...
        try:
            ...
            outline = ReportOutline(
                title=response.get("title", "平行人生报告"),
            ...
            return ReportOutline(
                title="平行人生报告",
                summary="基于平行宇宙模拟的另一种人生可能性探索",
                sections=[
                    ReportSection(title="平行世界的职业起点"),
                    ReportSection(title="人生轨迹分叉与关键选择"),
                    ReportSection(title="与现实人生的关键差异与启示")
                ]
            )
    ...
    def _generate_section_react(
        ...
        # 报告上下文，用于InsightForge的子问题生成（平行人生视角）
        report_context = f"章节标题: {section.title}\n平行人生场景: {self.simulation_requirement}"
